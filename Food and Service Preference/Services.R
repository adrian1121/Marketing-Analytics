...
#attributes
df <- read.csv("Services.csv")

#scree
sc <- scree(df[,5:8], factor=F)
fit <- principal(r=df[,5:8],nfactors=4,rotate="none")
fit

fit2 <- principal(r=df[,5:8],nfactors=2,rotate="varimax")
fit2
